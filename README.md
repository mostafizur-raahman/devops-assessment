...
## ☁️ AWS EC2 Setup (One-Time Manual Step)
...
- **`git reset --hard` over `git pull`:** In automated deployment scripts, `git pull` can fail if there are minor file permission changes or uncommitted local changes on the server. `git reset --hard` guarantees the server state is an exact, clean replica of the GitHub repository.
